...
const SyncSignup: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      let storedUsername = '';
      try {
        const res = await axios.post<{ username?: string }>(`${API_BASE_URL}/users/sync`, {
          clerkId: user.id,
        ...
        storedUsername = res.data?.username || '';
      } catch (_) {
        // Sync failure is non-fatal; user can still access the app
      } finally {
        setDone(true);
        navigate(storedUsername ? `/${storedUsername}` : '/user', { replace: true });
      }
    };
    sync();
  }, [user, done, navigate]);
    ...
    <ThemeProvider>
      <div className="min-h-screen bg-background text-on-background font-body" data-theme={isDarkTheme ? 'dark' : 'light'}>
        ...
        <aside className={`fixed left-0 top-0 h-screen w-64 bg-surface-container-low flex flex-col py-8 z-50 transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <div className="px-4 pb-4 pt-4 mt-auto border-t border-outline-variant">
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Created by</p>
            <p className="text-sm font-bold text-primary mb-2">Kumail Rizvi</p>
            <div className="flex items-center gap-3">
              <a href="https://www.linkedin.com/in/kumail-rizvi" target="_blank" rel="noreferrer" className="text-slate-500 hover:text-[#0077b5] transition-colors">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
              </a>
              <a href="https://devops-dojo.ninja" target="_blank" rel="noreferrer" className="text-slate-500 hover:text-primary transition-colors">
          ...
          <header className="flex justify-between items-center w-full px-4 lg:px-12 h-16 lg:h-20 bg-surface-container-low sticky top-0 z-40">
            ...
            <div className="flex items-center gap-3 lg:gap-6">
              ...
              <div className="flex items-center gap-3 text-slate-400">
                ...
                {userUsername && (
                  <span className="text-sm text-slate-400 font-medium">@{userUsername}</span>
                )}
              </div>
            </div>
          </header>
                            ...
                            if (isPast) {
                              return (
                                <div className="flex flex-col gap-2">
                                  <span className="text-[10px] font-bold uppercase px-3 py-2 text-center bg-slate-800 text-slate-500 cursor-not-allowed">
                                    📅 Google Calendar
                                  </span>
                                  <span className="text-[10px] font-bold uppercase px-3 py-2 text-center bg-slate-800 text-slate-500 cursor-not-allowed">
                                    📥 Download ICS
                          ...
                          <div className="bg-surface-container-high p-5 space-y-4">
                            ...
                            {isFuture && selectedSession.willRecord && (
                              <div className="space-y-2">
                                ...
                                <a href={`/recorder?session=${selectedSession.id}&author=${selectedSession.authorUsername || selectedSession.author}&title=${encodeURIComponent(selectedSession.topic)}&tags=${selectedSession.tag}`} target="_blank" rel="noreferrer"
