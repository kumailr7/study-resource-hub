...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => { if (materialRef.current) materialRef.current.uniforms.uTopColor.value = parseColor(topColor); }, [topColor]);
  useEffect(() => { if (materialRef.current) materialRef.current.uniforms.uBottomColor.value = parseColor(bottomColor); }, [bottomColor]);
  ...
  if (!webGLSupported) {
    return <div className={`light-pillar-fallback ${className}`} style={{ mixBlendMode: mixBlendMode as any }}>WebGL not supported</div>;
  }
